        ...
        <div>
          ...
          <div className="flex items-baseline space-x-3 mb-1">
            ...
            {product.originalPrice && (
                <span className="text-gray-400 font-semibold text-sm line-through">${product.originalPrice.toFixed(2)}</span>
            )}
          </div>
           ...
           <div className="flex items-center space-x-3 mb-2">
               <div className="flex text-orange-400">
                 {[...Array(5)].map((_, i) => (
                    i < Math.floor(product.rating) ? <IoIosStar key={i} /> : <IoIosStarOutline key={i} className="text-gray-300"/>
                 ))}
               </div>
